...
input_stack = vigra.readHDF5("./myWorkingDirectory/volume_big_stuff_classifier_also_from_C.h5", "data", order = 'C')
# mask_input = vigra.readHDF5("big_stuff_classifier_512_set_C_prediction_from_LSVM.h5", "data", order = 'C')
input_stack = vigra.dropChannelAxis(input_stack)
img = input_stack[6, :, :]
# vigra.impex.writeImage(exposure.equalize_hist(img).T, "test.png", dtype = numpy.uint8)
# result = numpy.zeros(input_stack.shape, dtype = numpy.uint8, order = 'C')
for i in range(input_stack.shape[0]):
    # input_stack[i, ...] = exposure.equalize_hist(input_stack[i, ...])
    vigra.impex.writeImage(exposure.equalize_hist(input_stack[i, ...]).T, "./equalize2/big_stuff" + str(i) + ".png", dtype = numpy.uint8)
...
img_eq = numpy.reshape(img_eq, (1, 3072, 3072), order = 'C')
