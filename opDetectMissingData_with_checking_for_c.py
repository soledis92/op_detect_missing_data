#!/home/adrian/miniconda2/envs/ilastik-devel/bin/python2.7
import pdb
...
class OpDetectMissing(Operator):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.positive_TrainingHistograms.setValue(_defaultTrainingHistograms())
    ...
    def _detectMissing(self, data):
        ...
        # walk over slices (z-axis of input data)
        for z in range(data.shape[0]):
            ...
            for patch in patches:
                hist = np.histogram(
                    ...
                    range=self._inputRange, density=True)[0]
                hists.append(hist)
    ...
    def train(self, force=False):
        ...
        f = open("c_vs_precision_recall.txt", mode = 'a')
        list_of_Cs = []
    ...
    def _felzenszwalbTraining(self, negative, positive, C_val):
        """
        ...
        def felzenstep(x, cache, ind):
            ...
            if len(easy_in_cache) > 0:
                (remove_from_cache, _, _, _) = _chooseRandomSubset(
            ...
            # grow the cache
            temp = len(cache)
            add_to_cache = _chooseRandomSubset(
    ...
    def _cross_validate(self, positive_histograms, negative_histograms, n_folds = 10, C_val = 1000):
        ...
        # necessary because the split up groups cant be accessed via indexing.
        ...
        for positive_trainig_group_indices, positive_testing_group_indices in positive_folds:
            for negative_trainig_group_indices, negative_testing_group_indices in negative_folds:
                ...
                if pos_counter == neg_counter:
                    ...
                    predictions_for_negative_sub_set = self.predict(negative_histograms[negative_testing_group_indices, 1],
                                                                    method = self.DetectionMethod.value)
                    predictions_for_positive_sub_set = self.predict(positive_histograms[positive_testing_group_indices, 1],
                                                                    method = self.DetectionMethod.value)
                    # get all positions of interest
                    predicted_detections_from_positive_sub_group = positive_histograms[np.where(predictions_for_positive_sub_set == 1), 0]
                    predicted_background_from_positive_sub_group = positive_histograms[np.where(predictions_for_positive_sub_set == 0), 0]
                    predicted_detections_from_negative_sub_group = negative_histograms[np.where(predictions_for_negative_sub_set == 0), 0]
                    predicted_background_from_negative_sub_group = negative_histograms[np.where(predictions_for_negative_sub_set == 1), 0]
                    # count how many are wrong pixelwise
                    wrongly_predicted_detections_positive_sub_group_pixelwise = self._count_wrong_pixelwise(predicted_detections_from_positive_sub_group, 1)
                    wrongly_predicted_background_positive_sub_group_pixelwise = self._count_wrong_pixelwise(predicted_background_from_positive_sub_group, 0)
                    wrongly_predicted_detections_negative_sub_group_pixelwise = self._count_wrong_pixelwise(predicted_detections_from_negative_sub_group, 0)
                    wrongly_predicted_background_negative_sub_group_pixelwise = self._count_wrong_pixelwise(predicted_background_from_negative_sub_group, 1)
                    # count how many are wrong objectwise
                    wrongly_predicted_detections_positive_sub_group_objectwise = self._count_wrong_objectwise(predicted_detections_from_positive_sub_group, 1)
                    wrongly_predicted_background_positive_sub_group_objectwise = self._count_wrong_objectwise(predicted_background_from_positive_sub_group, 0)
                    wrongly_predicted_detections_negative_sub_group_objectwise = self._count_wrong_objectwise(predicted_detections_from_negative_sub_group, 0)
                    wrongly_predicted_background_negative_sub_group_objectwise = self._count_wrong_objectwise(predicted_background_from_negative_sub_group, 1)
                    ...
                    precision_pixelwise = float(true_positives_pixelwise) / float(true_positives_pixelwise + false_positives_pixelwise)
                    recall_pixelwise = float(true_positives_pixelwise) / float(true_positives_pixelwise+ false_negatives_pixelwise)
                    ...
                    precision_objectwise = float(true_positives_objectwise) / float(true_positives_objectwise + false_positives_objectwise)
                    recall_objectwise = float(true_positives_objectwise) / float(true_positives_objectwise+ false_negatives_objectwise)
        ...
        #print "pixelwise:"
        #print "precision_pix: ", mean_precision_pixelwise, "+-", error_precision_pixelwise
        #print "recall_pix: ", mean_recall_pixelwise, "+-", error_recall_pixelwise

        #print "objectwise: "
        #print "precision_obj: ", mean_precision_objectwise, "+-", error_precision_objectwise
        #print "recall_obj: ", mean_recall_objectwise, "+-", error_recall_objectwise
    ...
    @classmethod
    def fit(cls, negative, positive, method='classic', C_val= 1000):
        """
...
if __name__ == "__main__":
    ...
    csvfile = open(os.path.join(
        working_directory, "%s_test_results.tsv" % (thisTime,)), 'w')
            ...
            for binSize in binSizes:
                ...
                if startFromLabels:
                    ...
                    trainHistogramExtractor_l2 = HistogramExtractor.HistogramExtractorFromLabels(
                        trainData, trainLabels, 2, patchSize, binSize, 5000)
                    trainHistogramExtractor_l3 = HistogramExtractor.HistogramExtractorFromLabels(
                        trainData, trainLabels, 3, patchSize, binSize, 5000)
                    ...
                    trainHistogramExtractor_l1 = HistogramExtractor.HistogramExtractorFromLabels(
                        trainData, trainLabels, 1, patchSize, binSize, 10000)
                    positive_train_histograms2 = trainHistogramExtractor_l2.getHistogramsAsList()
                    positive_train_histograms3 = trainHistogramExtractor_l3.getHistogramsAsList()
                    positive_train_histograms = positive_train_histograms2 + positive_train_histograms3
                    negative_train_histograms = trainHistogramExtractor_l1.getHistogramsAsList()
                    ...
                    for sample in negative_train_histograms:
                        ...
                        assert not np.any(np.isnan(sample[1]))
                ########################################################################################################
                # Fixme delete this later this is only for development of cross validation!!!
                # labels are needed to check if true/ false positive
                labels = None
                locations = ['/volume/data', '/cube']
                for location in locations:
                        try:
